...
import { useAddAppointmentDailogCtrl } from "./ctrl";

function AddUpdateAppointmentDailog({
  onClose,
  appointment,
}: {
  ...
  const ctrl = useAddAppointmentDailogCtrl({ onClose, appointment });
  return (
    <DialogContent
      onXClick={() => onClose(false)}
      onInteractOutside={() => onClose(false)}
    >
      <DialogTitle>{appointment ? "Update" : "Add"} Appointment Status</DialogTitle>
...
export default AddUpdateAppointmentDailog;
